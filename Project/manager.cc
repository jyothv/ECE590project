...
namespace elma {
    ...
    Manager& Manager::watch(std::string event_name, std::function<void(Event&)> handler, int priority) {
        //std::cout<<"mngr watch "<<event_name<<"\n";
        int index = 0;
        for ( auto check_priority : event_priority[event_name] ) {
            if(check_priority > priority){
                break;
            }
            index++;
        }
        event_priority[event_name].insert(event_priority[event_name].begin()+index, priority);
        event_handlers[event_name].insert(event_handlers[event_name].begin()+index, handler);
        return *this;
    }
    ...
    Manager& Manager::emit(const Event& event) {
        Event e = event; // make a copy so we can change propagation
         //std::cout<<"manager emit: "<<event.name()<<"\n";
        if ( event_handlers.find(event.name()) != event_handlers.end() ) {
            //std::cout<<"manager found event "<<event.name()<<"\n";
            for ( auto handler : event_handlers[event.name()] ) {
               
                if ( e.propagate() ) {
                //std::cout<<"propagate: "<<event.name()<<"\n";
                  handler(e);
                }
            }
        }
